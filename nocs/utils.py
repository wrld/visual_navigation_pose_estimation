...
# from nocs.aligning import estimateSimilarityTransform
...
def compute_degree_cm_mAP(npyname, final_results, synset_names, log_dir, category, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False):
    ...
    shift_thres_list = list(shift_thresholds) + [10000]
    num_shift_thres = len(shift_thres_list)
    ...
    for progress, result in enumerate(final_results):
        ...
        pred_RTs_ours = np.array(result['pred_RTs_ours'])
        ...
        for cls_id in range(1, num_classes):
            ...
            cls_pred_RTs_ours = pred_RTs_ours[pred_class_ids==cls_id] if len(pred_class_ids) else np.zeros((0, 4, 4))
            ...
            if len(iou_pred_indices):
                ...
                # Diff to NOCS: Use NOCS's prediction to determine which samples are used to evaluate R and T
                cls_pred_RTs_ours = cls_pred_RTs_ours[iou_pred_indices]
            ...
            if use_matches_for_pose:
                ...
                cls_pred_RTs_ours = cls_pred_RTs_ours[iou_thres_pred_match > -1] if len(iou_thres_pred_match) > 0 else np.zeros((0, 4, 4))
    ...
    print('5 degree, 100cm: {:.1f}'.format(pose_aps[-1, degree_thres_list.index(5),shift_thres_list.index(10000)] * 100))
...
def align_ICP(class_ids, masks, depth, intrinsics, synset_names, save_path=None, if_norm=False, verbose=False):
    num_instances = 1
    error_messages = ''
    ...
    for i in range(num_instances):
        ...
        glob_paths = './real_test/{}*.txt'.format(synset_names[class_ids])
        print(glob_paths)
