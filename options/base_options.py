...
from utils.visualizer.base_visualizer import BaseVisualizer as Visualizer
...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        # basic parameters
        basic_args = parser.add_argument_group('nocs_basic')
        ...
        # model parameters
        model_args = parser.add_argument_group('nocs_model')
        ...
        # dataset parameters
        data_args = parser.add_argument_group('nocs_data')
        data_args.add_argument('--dataset_mode', type=str, default='nocs_hdf5', help='chooses how datasets are loaded. [unaligned | aligned | single | colorization]')
        ...
        misc_args = parser.add_argument_group('nocs_misc')
    ...
    def gather_options(self, parent_parser):
        ...
        if not self.initialized:  # check if it has been initialized
            ...
            # parser = parent_parser.add_argument_group(title="nocs image generator")
            # subparsers = parent_parser.add_subparsers()
            # parser = parent_parser.add_parser('nocs', help='nocs help')
            parser = self.initialize(parent_parser)
    ...
    def parse(self, parent_parser):
        """Parse our options, create checkpoints directory suffix, and set up gpu device."""
        opt = self.gather_options(parent_parser)
