...
class LatentObjectModel(BaseModel):
    ...
    def fitting(self, real_B):
        ...
        for iter in tqdm.tqdm(range(self.opt.n_iter)):
            ...
            image_history = torch.cat([image_history, fake_B.cpu()], dim=0)
            ...
            # print(criterion(fake_B_upsampled[0:1, :, :, :], real_B[0:1, :, :, :]).view(
            #     1, -1).mean(1))
        # print(fake_B_upsampled.shape , real_B.shape)
        image_history = torch.cat([image_history, real_B.cpu()], dim=0)
